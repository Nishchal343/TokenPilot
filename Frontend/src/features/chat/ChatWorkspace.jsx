...
export default function ChatWorkspace({ embedded = false, initialPrompt = '' }) {
  ...
  const createChat = () => { setChat({ id: null, title: 'New conversation', messages: [] }); setCacheCandidate(null); inputRef.current?.focus() }
  ...
  const send = async (forced, options = {}) => {
    const content = (forced ?? prompt).trim(); if ((!content && !images.length && !documents.length) || loading) return
    ...
    try {
      const request = { content, images:outgoingImages.map(({ mime_type, data }) => ({ mime_type, data })), documents:outgoingDocuments, key_id:selected.id, key_source:selected.source, ...options }
      const response = target?.id ? await workspaceApi.send(target.id, request) : await workspaceApi.sendNew(request)
      const answer = { ...response.data, provider:selected.label, model:selected.model }
      setChat(current => ({ ...current, id:current.id || response.data.chat_id, title:current.title === 'New conversation' ? content.slice(0, 60) : current.title, messages:[...current.messages, answer] }))
