...
_CONTEXT_DEPENDENT_PROMPT = re.compile(r"\b(above|previous|earlier|continue|again|also|same|that|this)\b", re.IGNORECASE)
...
=========================================="""
    ...
    def generate(self, content: str, history: list[dict], images: list[dict], selected: ProviderCandidate, available: list[ProviderCandidate], documents: list[dict] | None = None, code_files: list[dict] | None = None, cache_scope: str | None = None) -> AIResult:
        ...
        cache_context = self.context_optimizer.select_context(cache_history, optimized_content, selected_provider, selected_model).messages
        ...
        try:
            ...
            cached = self.cache.lookup(cache_key, semantic_query=optimized_code_prompt, context_key=cache_context_key)
            if not cached:
        ...
        if cached:
            ...
            report["cache"] = {"status": "HIT", "api_call_avoided": True, "response_time_saved_ms": cached.provider_latency_ms}
        ...
        for candidate in candidates[:1]:
            try:
                ...
                report = optimization.as_report(candidate.provider, candidate.model)
                context_report = context.as_report(candidate.provider, candidate.model)
                report.update({key: value for key, value in context_report.items() if key != "stages"})
                report["stages"].update(context_report["stages"])
                report["context_messages_removed"] = context.messages_removed
                report["context_messages_preserved"] = context.messages_preserved
                report["context_optimization_ms"] = context.optimization_ms
                report["context_request_id"] = request_id
                code_report = code.as_report(candidate.provider, candidate.model)
                report.update({key: value for key, value in code_report.items() if key != "stages"})
                report["stages"].update(code_report["stages"])
                document_report = document.as_report(candidate.provider, candidate.model)
                report.update({key: value for key, value in document_report.items() if key != "stages"})
                report["stages"].update(document_report["stages"])
                report["prompt_original_tokens"] = report["original_tokens"]
                report["prompt_optimized_tokens"] = report["optimized_tokens"]
                ...
                report["prompt_cost_after"] = report["cost_after"]
                report["prompt_cost_saved"] = report["cost_saved"]
                report["document_original_tokens"] = int(report.get("document_original_tokens", 0))
                report["document_optimized_tokens"] = int(report.get("document_optimized_tokens", 0))
                report["document_tokens_saved"] = int(report.get("document_tokens_saved", 0))
                report["code_original_tokens"] = int(report.get("code_original_tokens", 0))
                report["code_optimized_tokens"] = int(report.get("code_optimized_tokens", 0))
                report["code_tokens_saved"] = int(report.get("code_tokens_saved", 0))
                module_original = report["prompt_original_tokens"] + report["original_context_tokens"] + report["document_original_tokens"] + report["code_original_tokens"]
                module_optimized = report["prompt_optimized_tokens"] + report["optimized_context_tokens"] + report["document_optimized_tokens"] + report["code_optimized_tokens"]
                module_saved = max(0, module_original - module_optimized)
                module_cost_before = report["prompt_cost_before"] + report["context_cost_before"]
                module_cost_after = report["prompt_cost_after"] + report["context_cost_after"]
                module_cost_before += report["document_cost_before"]
                module_cost_after += report["document_cost_after"]
                module_cost_before += report["code_cost_before"]
                module_cost_after += report["code_cost_after"]
                # These generic fields are the overall summary consumed by the
                # existing report/dashboard UI. Module-specific fields remain
                # available for the detail rows and future optimizers.
                report["overall_original_tokens"] = module_original
                report["overall_optimized_tokens"] = module_optimized
                report["overall_tokens_saved"] = module_saved
                report["overall_reduction_percent"] = round((module_saved / module_original) * 100, 2) if module_original else 0
                ...
                if cache_key and isinstance(answer, str) and answer.strip():
                    self.cache.store(cache_key, answer, candidate.provider, candidate.model, latency, report, semantic_query=optimized_code_prompt, context_key=cache_context_key)
